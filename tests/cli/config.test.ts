...
const fixturePaths = process.platform === "win32"
  ? {
      workspace: "F:/demo",
      otherWorkspace: "F:/other",
      worktreeRoot: "F:/g2m-state/worktrees",
      artifactRoot: "F:/g2m-state/artifacts",
      program: "C:/Program Files/nodejs/node.exe",
      mcode: "C:/Users/test/AppData/Roaming/npm/mcode.cmd",
    }
  : {
      workspace: "/tmp/g2m-demo",
      otherWorkspace: "/tmp/g2m-other",
      worktreeRoot: "/tmp/g2m-state/worktrees",
      artifactRoot: "/tmp/g2m-state/artifacts",
...
describe("G2M local config", () => {
  it("accepts trusted workspace and verification mappings", () => {
    const config = parseLocalConfig({
      ...
      workspaces: [{ workspace_id: "demo", path: fixturePaths.workspace }],
      verification_profiles: [
        {
          id: "test",
          workspace_id: "demo",
          description: "run tests",
          program: fixturePaths.program,
          args: ["test.mjs"],
          timeout_ms: 10_000,
        },
      ],
      worktree_root: fixturePaths.worktreeRoot,
      artifact_root: fixturePaths.artifactRoot,
      mcode_path: fixturePaths.mcode,
    });
  ...
  it("rejects duplicate workspace ids and duplicate scoped profile ids", () => {
    expect(() =>
      parseLocalConfig({
        protocol_version: "g2m.local-config.v1",
        workspaces: [
          { workspace_id: "demo", path: fixturePaths.workspace },
          { workspace_id: "demo", path: fixturePaths.otherWorkspace },
        ],
        verification_profiles: [],
        worktree_root: fixturePaths.worktreeRoot,
        artifact_root: fixturePaths.artifactRoot,
  ...
  it("rejects zero, negative, and unreasonably large runtime limits", () => {
    const base = {
      protocol_version: "g2m.local-config.v1",
      workspaces: [{ workspace_id: "demo", path: fixturePaths.workspace }],
      verification_profiles: [], worktree_root: fixturePaths.worktreeRoot, artifact_root: fixturePaths.artifactRoot,
    };
